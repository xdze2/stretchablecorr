...
for image_id, displ_field in enumerate(displ_lagrangian_to_ref):
    ...
    title =  f'{stretch_values[image_id]}% - '
    title += f'{image_names[image_id]} - '
    title += f'eps_zz [%] (nu={Poisson_coeff}) - '
    title += f'displ. scale x{view_factor}'
    plt.title(title);
    
    output_dir = 'eps_zz_maps' if points_mask is None else 'eps_zz_maps_mask'
    ft.save_fig(f'eps_zz_{image_id:03d}', sample_name, output_dir, close=True)
